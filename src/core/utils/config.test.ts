...
describe('config', (): void => {
    ...
    it('fetch config if not in local storage', async () => {
        ...
        const configValues = await config.fetchAndSetConfig();
        expect(configValues).toEqual(configValues);
        expect(window.fetch).toHaveBeenCalledTimes(1);
    });

    it('fetches config from local storage if avaliable', async () => {
        jest.spyOn(window, 'fetch').mockImplementation(() =>
            Promise.resolve(({
                ok: true,
                json: () => JSON.stringify(data),
        ...
        await config.fetchAndSetConfig();
