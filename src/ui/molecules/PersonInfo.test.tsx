...
describe('PersonInfo', (): void => {
    ...
    it('should render correctly', (): void => {
        expect((): RenderResult => render(<PersonInfo name="" aff="" expertises={[]} focuses={[]} />)).not.toThrow();
    });

    it('should render the name', (): void => {
        const testName = 'Test Name';
        const { getByText } = render(<PersonInfo name={testName} aff="" expertises={[]} focuses={[]} />);
        expect(getByText(testName)).toBeInTheDocument();
    });

    it('should render the institution', (): void => {
        const testInstitution = 'Test Institution';
        const { getByText } = render(<PersonInfo name="" aff={testInstitution} expertises={[]} focuses={[]} />);
    ...
    it('should render the expertises', (): void => {
        ...
        const { getByText } = render(<PersonInfo name="" aff="" expertises={testExpertises} focuses={[]} />);
        expect(getByText(testExpertises.join(', '))).toBeInTheDocument();
    });

    it('should render the focuses', (): void => {
        const testFocuses = ['focus 1', 'focus 2'];
        const { getByText } = render(<PersonInfo name="" aff="" expertises={[]} focuses={testFocuses} />);
